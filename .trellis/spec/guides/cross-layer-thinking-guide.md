...
### Mistake 5: Treating Hosted and Packaged Providers as One Contract

An MCP provider's hosted endpoint and its npm/stdio package may share tool names while returning
different payload shapes or versions. Package source proves only the packaged deployment; it does
not freeze the hosted result schema.

For every external deployment used by the application:

- bind contract evidence to the exact transport and endpoint;
- prefer current first-party documentation for that deployment;
- do not treat a capability table's semantic output fields as an exact wire schema;
- keep raw external validation separate from internal normalization;
- when no typed schema/example exists, authorize a structure-only key/type probe before mapping;
- require a separately authorized live proof before marking the deployment externally accepted.

### Mistake 6: Inferring Raw Shape From A Safe Validation Path

A redacted Zod path such as `content.0.text` identifies where validation failed; it does not prove that the envelope was missing, that `structuredContent` was present, or what the rejected value contained.

Before adding response compatibility:

- inspect the exact pinned deployment package, official fixture, or separately authorized structure evidence;
- classify envelope validation separately from payload validation;
- add only the narrow deployment-specific rule supported by that evidence;
- keep unknown shapes fail-closed and require fresh LIVE evidence before declaring recovery.
